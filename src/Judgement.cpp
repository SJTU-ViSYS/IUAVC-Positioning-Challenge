...
Judgement::Judgement(ros::NodeHandle &_nh) : mNodeHandle(_nh)
{
    ...
    std::string jsonFilePath,sensorsStr;
    ros::param::get("/refree/team", mstrTeam);
    ros::param::get("/refree/sensors", sensorsStr);
    ROS_INFO("Use %s as major topic.",sensorsStr.c_str());
    if(sensorsStr=="/front/rgbd"){
        ...
        mstrMajorImgTopic="/back/fisheye1/image_raw";
    }
    std::string outFilePath = mstrOutFilePathBase + mstrTeam + "_result_traj.txt";
...
double Judgement::JudgeRealTime()
{
    ...
    for (auto &msg : mlMsgRec)
    {
        ...
        if (StampDuration(imageStamp, publisherImageStamp) < msec2nsec(1) && duration < msec2nsec(mdRealtimePerformanceTh))
        {
            ...
            mfOutFile << std::fixed << std::setprecision(18) << imageStamp << "," << std::setprecision(15)
                      << msgPtr->pose.pose.position.x << "," << msgPtr->pose.pose.position.y << "," << msgPtr->pose.pose.position.z << ","
                      << msgPtr->pose.pose.orientation.w << "," << msgPtr->pose.pose.orientation.x << "," << msgPtr->pose.pose.orientation.y << "," << msgPtr->pose.pose.orientation.z
                      << std::endl;
            countValidFrame++;
