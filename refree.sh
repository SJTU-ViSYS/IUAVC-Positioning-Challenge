...
rosbag_path=$(pwd)/dataset/IntelligentUAV_${seq}.bag
if [ -d "${output_path}" ]; then rm -rf ${output_path}; fi
mkdir -p ${output_path}
source ../../devel/setup.bash;
roslaunch stage4_refree refree.launch output_path_base:=${output_path} team:=${team} sensors:=${sensors} rosbag_path:=${rosbag_path}
if [ -f "${output_path}/${team}_result.json" ]; then
    if [ -f "${output_path}/${team}_result_traj.txt" ]; then 
        ...
        python3 thirdparty/stage4_evo/evo/main_rpe.py tum ./GroundTruth/${seq}_gt.tum ${output_path}/${team}_result_traj.tum -a --save_results ${output_path}/${team} -p 
    fi
fi
